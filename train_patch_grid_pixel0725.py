...
class PatchTrainer(object):
    ...
    def train(self):
        ...
        for img_name in img_list:
            ...
            segments_mini = get_grid_mini(img_tensor_for_slic, width=10) + 1
            ...
            with torch.no_grad():
                ...
                for reg_num_index in tqdm(range(len(unique_segments_cover))):
            ...
            img_tmp = resize_small(img_tmp).cuda().unsqueeze(0)
            ...
            for step in range(unique_segments_num):
                ...
                with torch.no_grad():
                    ...
                    sp_layer_now608 = resize_small(sp_layer_now)
                    mask_detected608 = resize_small(mask_detected)

                    img_tmp = torch.where(
                        (sp_layer_now608.repeat(3, 1, 1) >= 0.1).mul(mask_detected608.repeat(3, 1, 1) >= 0.1),
                        gray_img608, resize_small(img_batch.clone())).cuda()
                    # img_tmp = resize_small(img_tmp).cuda()
                    boxes = do_detect(self.darknet_model, img_tmp, 0.4, 0.4, True)
                    old_boxes = boxes.copy()
                    old_boxes_tensor = torch.Tensor(old_boxes)
                    ...
                    for selected_node in select_list:
                        ...
                        # grid
                        neighbor_list = get_grid_neighbor(segments_mini, selected_node, unique_segments_cover)
                        for nei in neighbor_list:
                            now_neighbor_list.append(nei)
                    ...
                    for i1 in range(now_neighbor_unique_np.size):
                        ...
                        #     fgsm_img, img_batch.clone())
                        sp_layer_tmp608 = resize_small(sp_layer_tmp)
                        osp_img = torch.where(
                            (sp_layer_tmp608.repeat(3, 1, 1) >= 0.1).mul(mask_detected608.repeat(3, 1, 1) >= 0.1),
                            gray_img608, resize_small(img_batch.clone()))
                        osp_img_gpu = resize_small(osp_img).cuda().unsqueeze(0).cuda()
