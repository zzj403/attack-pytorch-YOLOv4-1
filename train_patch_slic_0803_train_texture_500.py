        ...
        for img_name in img_list:
            ...
            with torch.no_grad():
                ...
                for reg_num_0 in unique_segments_cover:
                    ...
                    cv2_dilation_ = torch.from_numpy(cv2_dilation_)
                ...
                graph_output_path = os.path.join('graph_save', img_name+'_graph.pkl')
                ...
                with open(graph_output_path, 'rb') as file:
                ...
                ## start at center
                select_list = []
                for i_b in range(len(original_box_list)):
                ...
                for reg_num_index in range(len(unique_segments_cover)):
                    ...
                    # show the img
                    # if reg_num_index == 2:
                    #     osp_img_gpu = resize_608(osp_img).cuda().unsqueeze(0)
            ...
            # old_boxes = boxes.copy()
            # old_boxes_tensor = torch.Tensor(old_boxes)
            output_before = self.darknet_model(img_tmp)
            max_prob_before = self.prob_extractor(output_before).cpu()
            fgsm_loss = F.binary_cross_entropy(input=max_prob_before, target=torch.tensor([1.]), size_average=False)
            self.darknet_model.zero_grad()
            fgsm_loss.backward()
            data_grad = img_tmp.grad.data

            # call FGSM attack
            epsilon = 0.3
            perturbute_data2 = fgsm_attack2(epsilon, data_grad).squeeze()
            perturbute_data = fgsm_attack(img_tmp, epsilon, data_grad).squeeze()
            fgsm_img = perturbute_data.cpu()

            img = perturbute_data.squeeze()
            img = transforms.ToPILImage()(img.detach().cpu())
            # img.show()
            boxes222 = do_detect(self.darknet_model, perturbute_data, 0.4, 0.4, True)
            class_names = load_class_names('data/coco.names')
            plot_boxes(transforms.ToPILImage()(perturbute_data.detach().cpu().squeeze()), boxes222,
                       'test2/predictions_' + str(epsilon) + '.jpg',
                       class_names)
            ...
            for step in range(unique_segments_num):
                ...
                with autograd.detect_anomaly():
                    ##refresh the original boxes
                    sp_layer_now608 = resize_608(sp_layer_now)
                    ...
                    # img_tmp = torch.where(
                    #         (sp_layer_now.repeat(3, 1, 1) == 1).mul(mask_detected.repeat(3, 1, 1) == 1),
                    #         fgsm_img, img_batch.clone())
                    # img_tmp = resize_608(img_tmp).cuda()

                    boxes222 = do_detect(self.darknet_model, img_tmp, 0.4, 0.4, True)
                    class_names = load_class_names('data/coco.names')
                    ...
                    for i1 in range(now_neighbor_unique_np.size):
                        ...
                        sp_layer_tmp = torch.where((segments_tensor == now_node).mul(mask_detected == 1),
                                                   white_img_single_layer,
                                                   sp_layer_now)
                        # img = sp_layer_tmp
                        # imgshow = transforms.ToPILImage()(img.squeeze().detach().cpu())
                        ...
                        # if now_node == 562:
                        #     img = torch.where((segments_tensor == now_node).mul(mask_detected == 1),
                        #                            white_img_single_layer,
                        #                            img_batch)
                        #     img = transforms.ToPILImage()(img.detach().cpu())
                        #     img.show()

                        sp_layer_tmp608 = resize_608(sp_layer_tmp).squeeze()
                    ...
                    # sp_layer_tmp_for_show = torch.where((segments_tensor == select_superpixel_no).mul(mask_detected == 1),
                    #                            white_img_single_layer,
                    #                            img_batch)
                    # img = sp_layer_tmp_for_show
                    # img = transforms.ToPILImage()(img.detach().cpu())
                    # img.show()

                    sp_layer_old = sp_layer_now.detach().clone()
                    sp_layer_now = torch.where((segments_tensor == select_superpixel_no).mul(mask_detected == 1),
                                               white_img_single_layer,
                                               sp_layer_now)

                    sp_layer_area = torch.sum(sp_layer_now)
                    print('Now best attack =', float(torch.min(torch.tensor(score_m_list))))
                    if sp_layer_area > 5000:
                        img = sp_layer_old
                        img = transforms.ToPILImage()(img.detach().cpu())
                        img.save(os.path.join('black_superpixel/2', img_name.split('.')[0] + '_final.png'))
                        break
                    if torch.min(torch.tensor(score_m_list)) < 0.3:
                        img = sp_layer_now
                        img = transforms.ToPILImage()(img.detach().cpu())
                        img.save(os.path.join('black_superpixel/2', img_name.split('.')[0] + '_final.png'))
                        break
                    ...
                    # img = transforms.ToPILImage()(img.detach().cpu())
                    # img.save(os.path.join('black_superpixel_img', img_name.split('.')[0]+'_'+str(step)+'.png'))
                    ...
                    output_before = self.darknet_model(resize_608(img_now).unsqueeze(0).cuda())
                    max_prob_before = self.prob_extractor(output_before).cpu()
                    ...
                    ## 2
                    # boxes = do_detect(self.darknet_model, resize_608(img_now).unsqueeze(0).cuda(), 0.4, 0.4, True)
                    ...
                    img_now_gpu = resize_608(img_now).cuda().unsqueeze(0)
