...
class PatchTrainer(object):
    ...
    def train(self):
        ...
        dataset = COCODataset(model_type='YOLOv4',
                              data_dir='data/coco/',
                              img_size=img_size,
                              augmentation=None, # shu ju zeng qiang
                              debug=False)
        ...
        for epoch in range(n_epochs):
            ...
            ep_loss = 0
            bt0 = time.time()
            for i_batch, (img_batch, lab_batch, info, id_) in tqdm(enumerate(train_loader), desc=f'Running epoch {epoch}',
                                                        total=self.epoch_length):
                with autograd.detect_anomaly():
                    img_batch = img_batch.cuda()
                    lab_batch = lab_batch.cuda()
                    lab_batch = lab_batch[:,:10,:]
                    for i1 in range(lab_batch.shape[0]):
                        for j1 in range(lab_batch.shape[1]):
                            if lab_batch[i1,j1,0] > 0:
                                lab_batch[i1,j1,0]=0
                            if sum(lab_batch[i1,j1]) ==0:
                                lab_batch[i1,j1,0] = 1
                    ...
                    img.astype(np.uint8)
                    cv2.imwrite('img_3.jpg', img)  # save as BGR
                    ...
                    # img = p_img_batch[0, :, :,]
                    # img = transforms.ToPILImage('RGB')(img.detach().cpu())
                    # img.show()
